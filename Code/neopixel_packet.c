...
static PT_THREAD (protothread_timer(struct pt *pt)){
    ...
    static  unsigned char NeoPixel, color_change, color_seq ;
      ...
      while(1) {
        ...
        if (dir==1) position++;
        if (dir==-1) position--;
        if (position == NeoNum-pulse_size) dir=-1;
        else if (position == 0) dir=1;
        
        // mutate color template
        color_change++;
        //if((color_change & 0x3f) == 0){
        if((position == 0) || (position==NeoNum-pulse_size)){
            color_seq++ ;
            for (NeoPixel = 0; NeoPixel < NeoNum; NeoPixel++)   
            {
               if (NeoPixel > pulse_size){
                    NeoGreenTemplate[NeoPixel] =  64 * (((color_seq & 0x03)==1) || ((color_seq & 0x03)==0) );
                    NeoBlueTemplate[NeoPixel]  =  64 * (((color_seq & 0x03)==2) || ((color_seq & 0x03)==0) ); 
                    NeoRedTemplate[NeoPixel]   =  64 * (((color_seq & 0x03)==3) || ((color_seq & 0x03)==0) ); 
               }
            }
        }
